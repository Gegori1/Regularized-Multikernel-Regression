...
from sklearn.gaussian_process.kernels import (
    Matern, ExpSineSquared, RationalQuadratic, WhiteKernel, ConstantKernel, RBF
)
...
parameter_path = os.path.join("average_sklearn.jsonl")
parameters = []
...
def process_split(train_index, test_index, X_, y_, scaler, parameter):
    ...
    kern = (
        0.47153789900802573 * ExpSineSquared(periodicity=132 / scaler.scale_[0], length_scale=1e-1) +
        0.08121753570093063 * ExpSineSquared(periodicity=11 / scaler.scale_[0]) +
        0.020414233254392185 * ExpSineSquared(periodicity=11 / scaler.scale_[0], length_scale=1e-1) +
        0.00034905437735755076 * ExpSineSquared(periodicity=1 / scaler.scale_[0]) +
        0.10134318272716829 * Matern(nu=0.5, length_scale=1e-1) +
        0.001508544083790035* WhiteKernel()         
    )
